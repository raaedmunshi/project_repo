...
public class UpdateDeleteStudent extends javax.swing.JFrame {
    ...
    private void getNewData() {
        ...
        try {
            String str;
            rs = mycon.getstate().executeQuery("SELECT * FROM STUDENTS ORDER BY s_id ASC ");
            rs.beforeFirst();
            rs.first();
            populateFields();
    ...
    private void populateFields() {
        try {
            txtID.setText(rs.getString("s_id"));
    ...
    boolean isValidData() {
        ...
        if (txtName.getText().trim().isEmpty() || (txtName.getText().trim().length() > 10)) {
            ...
            } else if ((txtName.getText().trim().length() > 40)) {
                NameError.setText("Invalid. Must be < 40 chars.");
            }
        ...
        if (txtStanding.getText().trim().isEmpty() || (txtStanding.getText().trim().length() > 10) ) {
            ...
            else if(txtStanding.getText().trim().length() > 15)
                StandingError.setText("Invalid. Must be < 15 chars.");
    ...
    private void btnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        ...
        try {
            ...
            if (isValidData()) {
                findStanding();
                prepStatement = mycon.getCon().prepareStatement("UPDATE students name=?, gender=?, major=?, credits=?, standing=?, gpa=? where s_id = ?");
                ...
                prepStatement.setString(3, cmbMajor.getSelectedItem().toString());
                prepStatement.setInt(4, Integer.parseInt(txtCredits.getText()));
                prepStatement.setString(5, txtStanding.getText());
                prepStatement.setDouble(6, Double.parseDouble(txtGPA.getText()));
                prepStatement.setInt(7, Integer.parseInt(txtID.getText()));
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        ...
        try {
            // make the result set scrolable forward/backward updatable
            prepStatement = mycon.getCon().prepareStatement("DELETE students WHERE s_id = " + txtID.getText().trim());
            int result = prepStatement.executeUpdate();
            if (result > 0) {
